...
# openstack.org/telemetry-operator-bundle:$VERSION and openstack.org/telemetry-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= devopsartifact.jio.com/ind-indradhanus_app_dev_dcr/sandy007x/telemetry-operator
...
DEFAULT_IMG ?= devopsartifact.jio.com/ind-indradhanus_app__dev__dcr/sandy007x/telemetry-operator:v0.0.3
IMG ?= $(DEFAULT_IMG)
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	GOWORK=off GOFLAGS="-mod=mod" $(CONTROLLER_GEN) rbac:roleName=manager-role crd webhook paths="./..." output:crd:artifacts:config=config/crd/bases && \
	rm -f api/bases/* && cp -a config/crd/bases api/

.PHONY: generate
generate: controller-gen ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject method implementations.
	GOWORK=off GOFLAGS="-mod=mod" $(CONTROLLER_GEN) object:headerFile="hack/boilerplate.go.txt" paths="./..."
...
undeploy: ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	kubectl delete -f config/samples/deploy.yaml
